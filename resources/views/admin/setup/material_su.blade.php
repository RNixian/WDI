...
<form action="{{ route('admin.setup.storeunder_out_cat') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="w-full lg:w-1/2">
            <label class="block text-gray-700 font-bold mb-2" for="out_cat_id">Classification</label>
            <select name="out_cat_id" id="out_cat_id"
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
                <option value="">-- Select Output Classification --</option>
                @foreach ($Classification as $class)
                    <option value="{{ $class->id }}" {{ old('out_cat_id') == $class->id ? 'selected' : '' }}>
                        {{ $class->item_class }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="w-full lg:w-1/2">
            <label class="block text-gray-700 font-bold mb-2" for="item_category">Category</label>
...
<div class="mt-8"></div>
  ...
  <!-- Search and Filter -->
 <form method="GET" action="{{ url('/admin/setup/material_setup') }}" id="searchForm" class="w-full mb-6 flex flex-col md:flex-row items-center gap-2">
  <input
    type="text"
    name="search"
  ...
  <a
    href="{{ url('/admin/setup/material_setup') }}"
